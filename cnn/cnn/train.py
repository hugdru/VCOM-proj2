...
def create_models(num_classes, weights='imagenet', multi_gpu=0):
    ...
    if multi_gpu > 1:
        with tf.device('/cpu:0'):
            model = ResNet50RetinaNet(
                image, num_classes=num_classes, weights=weights, nms=False)
    ...
    prediction_model = keras.models.Model(
        inputs=model.inputs, outputs=model.outputs[:2] + [detections])
...
def create_callbacks(model, training_model, prediction_model,
    ...
    if args.snapshots:
        checkpoint = keras.callbacks.ModelCheckpoint(
            os.path.join(
                args.snapshot_path,
                'resnet50_{dataset_type}_{{epoch:02d}}.h5'.format(
